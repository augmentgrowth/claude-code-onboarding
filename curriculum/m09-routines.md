...
5. **Optional: set up the terminal command (only if the user wants scheduling).** Local scheduled runs need the `claude` terminal command, which the Desktop app does not include. If and only if the user wants to head toward scheduling, propose the install in plain words — "one small official install adds a `claude` command so routines can run on a schedule later; nothing else changes" — and after a yes in chat, install through a package manager only:
   - macOS, if Homebrew is present (check quietly with `which brew`): run `brew install --cask claude-code` yourself and show the result.
   - Windows: run `winget install Anthropic.ClaudeCode` yourself and show the result (winget ships with Windows).
   - No package manager available: do **not** run any installer another way, and never run or paste a command that pipes a download into a shell. Instead, send the user to the official install page in their own browser — https://code.claude.com/docs/en/setup — to copy the installer command for their platform themselves and paste it into their own Terminal, with one breath of expectation-setting first. Their machine, their command, from the official page.
   Verify with `claude --version` in a fresh shell; if it fails only with command-not-found, one fresh-terminal check before recording anything. A skipped or deferred install never blocks M9 — the manual rhythm in step 4 is the module's real product. Record the outcome in the ledger notes.
6. **Scheduling comes later, and only deliberately.** Manual-first is the rule: no scheduled or automated execution until the user has run the routine by hand at least once, and then only with explicit approval. Scheduling options change and differ in what they can reach — some scheduled runs happen in the cloud and cannot see the local vault at all — so the user checks the current official scheduling documentation in their own browser before choosing, and any routine that touches the vault must run where the vault lives. Record whatever gets enabled in the ledger with what it does, when it runs, and where it logs.
...
- The step-5 terminal-command install: proposed in plain words, approved in chat, package manager only. Administrator credentials are never requested; a credential wall means defer with an it-blocked note.

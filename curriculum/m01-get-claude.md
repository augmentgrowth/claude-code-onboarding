...
5. **Quick model check — set it, don't quiz it.** If session zero already set the model, skip this. Otherwise direct one concrete action: click the small model dropdown in the bottom-right corner of the message box, next to the send button, and pick Sonnet at medium effort — the right tool for setup. If they can't find the control, keep going; this check never blocks setup. Don't detour into plans, pricing, or usage commands here — a usage-limit stop later is a normal pause, not a failure: record true state in the ledger and give the hand-off.
